...
all2 <- read.table("normalization_gene_matrix.with_annotation.txt",sep="\t",head=TRUE,row.names=1)
...
test_genes$gene_id %>% 
   goana(x$entrezgene, species = "Mm", trend = TRUE) %>%
   topGO() %>%
         filter(P.DE < 0.05)

test_genes$gene_id %>%
   kegga(x$entrezgene, species.KEGG="mmu") %>% 
         arrange(P.DE) %>%
         filter(P.DE < 0.05)

```

Now get a vector of your DE genes and replicate what I've done here with the goana and kegga functions. Unfortunately, I havent got a way of getting these entrez gene ids out of our data as yet, so you'll have to look each gene up (from John's paper) mannually on the NCBI Gene website and get the ID from there 

```
genes <- c("entrez id 1", "entrez id 2", "entrez id 3") etc
```
